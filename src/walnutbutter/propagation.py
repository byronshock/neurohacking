"""Wave-by-wave signal propagation driven by a message queue of waves.

Firing a neuron does not call its neighbours, so there is no recursion and no
recursion-depth limit. Instead each active outgoing connection is queued for
the next wave, and the queue is one list of connections per wave: the
signals that the neurons firing in wave n send are delivered in wave n+1.

A wave is processed in two phases:

1. deliver every signal for that wave, adding weights to target potentials;
2. fire every neuron whose potential now meets its threshold.

Because all deliveries finish before any firing decision is made, the result
cannot depend on the order in which neurons or connections are stored.
Wave 0 is the external stimulus that starts the epoch.

Every neuron and every connection stays an object that does its own
receiving and firing; the queue is just a list per wave, so propagating a
signal costs no allocation beyond the wave record itself.
"""

from __future__ import annotations

import itertools
from typing import Iterable, Mapping

from .connection import Connection
from .neuron import Neuron

_wave_stamps = itertools.count(1)  # a fresh stamp per wave, across all propagations


class Signal:
    """A record of one delivery: the connection, the wave it was delivered in, its target and amount.

    Built on request by `Wave.signals()` for inspection and learning; the
    propagation itself queues bare connections.
    """

    __slots__ = ("connection", "wave", "target", "amount")

    def __init__(self, connection: Connection, wave: int):
        self.connection = connection
        self.wave = wave
        self.target = connection.target
        self.amount = connection.weight

    def __repr__(self) -> str:
        return f"Signal({self.connection!r}, wave {self.wave})"


class Wave:
    """What happened in one wave: the connections whose signals were delivered, and the neurons that fired."""

    __slots__ = ("number", "delivered", "fired")

    def __init__(self, number: int, delivered: list[Connection] | None = None, fired: list[Neuron] | None = None):
        self.number = number
        self.delivered: list[Connection] = [] if delivered is None else delivered
        self.fired: list[Neuron] = [] if fired is None else fired

    def signals(self) -> list[Signal]:
        """The deliveries of this wave as Signal records."""
        return [Signal(connection, self.number) for connection in self.delivered]

    def __eq__(self, other) -> bool:
        return (
            isinstance(other, Wave)
            and (self.number, self.delivered, self.fired) == (other.number, other.delivered, other.fired)
        )

    def __repr__(self) -> str:
        return f"Wave(number={self.number}, delivered={self.delivered!r}, fired={self.fired!r})"


def propagate(
    fire: Iterable[Neuron] = (),
    inputs: Mapping[Neuron, float] | None = None,
) -> list[Wave]:
    """Run one epoch and return its waves.

    `fire` lists neurons forced to fire in wave 0 regardless of threshold (an
    external stimulus). `inputs` maps neurons to external input amounts, which
    are delivered in wave 0 and fire the neuron only if it reaches threshold.
    """
    waves: list[Wave] = []

    # Wave 0: the external stimulus. Deliver all inputs first, then fire.
    wave = Wave(0)
    touched: list[Neuron] = []
    for neuron, amount in (inputs or {}).items():
        neuron.receive(amount)
        touched.append(neuron)
    pending: list[Connection] = []  # the next wave's queue: connections whose signals are in flight
    for neuron in fire:
        if not neuron.has_fired:
            _fire(neuron, wave, pending)
    _fire_ready(touched, wave, pending)
    waves.append(wave)

    # Later waves: each drains the queue the previous wave filled.
    while pending:
        wave = Wave(len(waves), pending)
        stamp = next(_wave_stamps)
        touched = []
        for connection in pending:
            target = connection.target
            target.receive(connection.weight)
            if target.touched_stamp != stamp:  # each neuron once per wave, without a set
                target.touched_stamp = stamp
                touched.append(target)
        pending = []
        _fire_ready(touched, wave, pending)
        waves.append(wave)

    return waves


def _fire_ready(candidates: Iterable[Neuron], wave: Wave, pending: list[Connection]) -> None:
    """Fire each candidate whose potential has reached its threshold.

    Candidates from a wave are already unique; the stimulus list in wave 0 may
    repeat a neuron, which is harmless because a fired neuron is never ready.
    """
    for neuron in candidates:
        if neuron.ready:
            _fire(neuron, wave, pending)


def _fire(neuron: Neuron, wave: Wave, pending: list[Connection]) -> None:
    """Fire one neuron and queue its outgoing signals for the next wave."""
    pending.extend(neuron.fire(wave.number))
    wave.fired.append(neuron)
