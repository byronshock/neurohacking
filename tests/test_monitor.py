...
def test_cli_learn_runs_epochs_and_reports_accuracy(capsys):
    ...
    assert cli_main(args) == 0
    err = capsys.readouterr().err
    assert "learning all-off (perturb, lr 0.1, sigma 0.1, homeostasis 1e-05 toward 0.4 in [-5, 5]): accuracy" in err
...
def test_cli_eligibility_and_sigma_options(capsys):
    ...
    assert cli_main(args) == 0
    assert "learning reversed (hebb, lr 0.02, sigma 0, homeostasis 1e-05 toward 0.4 in [-5, 5])" in capsys.readouterr().err
